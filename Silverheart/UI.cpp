...
void UI::setup()
{
	...
	markedForRemoval = new std::vector< std::string >;
}

void UI::update()
{
	//Remoing marked windows
	for(unsigned int i = 0; i < markedForRemoval->size(); i++)
	{
		if(getWindowExists(markedForRemoval->at(i)) == true)
		{
			removeWindow(markedForRemoval->at(i));
		}
	}
...
void UI::markWindowForRemoval(std::string windowID)
{
	markedForRemoval->push_back(windowID);
}
...
void List::updateInput(float mx, float my)
{
	...
	if(mx > startX && my > startY && mx < endX && my < endY)
	{
		for(unsigned int i = 0; i < listItems->size(); i++)
		{
			if(clickable == true)
			{
				listItems->at(i).update(mx, my, width, rowHeight);
			}
		}
	}
}
...
void List::setClickable(bool clickable)
{
...
void ListItem::update(float mx, float my, float width, float height)
{
	float x1 = x;
	float y1 = y;
	float x2 = x + width;
	...
	float screenX = agk::WorldToScreenX(mx);
	float screenY = agk::WorldToScreenY(my);

	if(screenX > x1 && screenY > y1 && screenX < x2 && screenY < y2)
	{
		for(unsigned int i = 0; i < cells->size(); i++)
		{
			cells->at(i).setColor(200, 200, 255);

			if(Input::click())
			{
				//Running the lua script
				LuaHandler::runFunction(onClickFile, onClickFunction);
			}
		}
	}
	else
	{
		for(unsigned int i = 0; i < cells->size(); i++)
		{
			cells->at(i).setColor(255, 255, 255);
		}
	}
}
...
void ListItem::setOnClick(std::string file, std::string function)
{
...
void ListCell::setColor(int r, int g, int b)
{
	agk::SetTextColor(this->TID, r, g, b, 255);
}
